...
class abb
{
  ...
  public:
    ...
    {
        if (pt->chave == value)
        { // Busca o valor na árvore, antes de adicionar
            cout << "erro-insert: Valor [" << value << "] já está na árvore!\n";
            return;
        }
        else if (pt->chave > value)
        { // Inserir a esquerda
            if (pt->esq != nullptr)
    ...
    void remove(int value)
    {
        ...
        { // Caso o nó não exista na árvore, eu encerro
        ...
        while (no != nullptr)
        {
            ...
            if (value < no->chave)
            { // Se estiver na esquerda da subarvore
                pai = no;
                no = no->esq;
                esquerdo = true;
            }
            else if (value > no->chave)
            { // Se estiver na direita da subarvore
                pai = no;
                no = no->dir;
                esquerdo = false;
            }
            else
            { // Se o nó atual eh o que estava sendo buscado
                break;
            }
        }
        ...
        if (no->dir == nullptr)
        { // Se só tem subarvore na esquerda
            if (esquerdo)
            {
        ...
        { // Se só tem subarvore na direita
            if (esquerdo)
            {
        ...
        {                                              // Caso 3 do slide (com duas subárvores)
    ...
    /**
     * Função auxiliar da função remove(). A função percorre a subarvore até chegar no maior elemento salvando o ponteiro pai dele
     */
    ...
    void print_recursive(node *no)
    {
        ...
        if (no->esq == nullptr && no->dir == nullptr) // Caso o nó seja folha
            ...
            ; // Caso o nó não tenha filho na direita
        else if (no->esq == nullptr && no->dir != nullptr)
            ; // Caso o nó não tenha filho na esquerda
    ...
    node *nth_element_recursive(node *no, int n)
    {
        ...
        { // Caso a árvore seja vazia ou o elemento não esteja na árvore
    ...
    int size(node *no)
    {
        if (no == nullptr) // Se o nó for nulo, tamanho é 0
            return 0;

        return no->tam; // Retorna o tamanho do nó
    }
    ...
    bool is_full_recursive(node *no)
    {
        ...
        // Caso os filhos da esquerda e direita não sejam nulos, confere se as subárvores destes filhos são cheias
    ...
    {
        node *no = raiz;
        int altura = height(raiz); // Calculo a altura da árvore
    ...
    {
        if (altura <= 2) // Caso a altura seja 0, 1 ou 2, siginfica que eh completa
            return true;

        if (no == nullptr) // Caso a árvore seja vazia
            return false;

        // Chamo a recursão com altura-1 pois disconsidero o próprio nó
        return is_complete_recursive(no->esq, altura - 1) && is_complete_recursive(no->dir, altura - 1);
    }

    /**
     * Função auxiliar da função is_complete(). Calcula o tamanho da árvore
     */
    int height(node *no)
    {
        if (no == nullptr)
        ...
        // Retorno sempre o valor maximo entre a altura da subárvore esquerda e da direita, e somo 1 pois conto com o próprio nó em que estou
    ...
    string to_string()
    {
        ...
        if (raiz == nullptr)
        { // Confiro antes se a árvore está vazia
